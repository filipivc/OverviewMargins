...
namespace Microsoft.VisualStudio.Extensions.MarkersToMarks
{
  ...
  [Export(typeof(IOverviewMarkFactoryProvider))]
    ...
    internal sealed class MarkFactoryProvider : IOverviewMarkFactoryProvider
    {
        ...
        {
            private static OLE.Interop.IServiceProvider _globalServiceProvider;
            private static IVsFontAndColorUtilities _fontAndColorUtilities;

            internal static OLE.Interop.IServiceProvider GlobalServiceProvider
            {
                get
                {
                    if (_globalServiceProvider == null)
                        _globalServiceProvider = (OLE.Interop.IServiceProvider)(Package.GetGlobalService(typeof(OLE.Interop.IServiceProvider)));
            ...
            internal static InterfaceType GetService<InterfaceType, ServiceType>(OLE.Interop.IServiceProvider serviceProvider)
            {
                return (InterfaceType)GetService(serviceProvider, typeof(ServiceType).GUID, false);
            }

            internal static object GetService(OLE.Interop.IServiceProvider serviceProvider, Guid guidService, bool unique)
            {
            ...
            {
                UInt32 w32color;
                Marshal.ThrowExceptionForHR(FontAndColorUtilities.GetRGBOfIndex(color, out w32color));
